...
client.connect(err => {
    ...
    const reviewCollection = client.db(`${process.env.DB_NAME}`).collection(`${process.env.DB_REVIEW_COLLECTION}`);
    const categoryCollection = client.db(`${process.env.DB_NAME}`).collection(`${process.env.DB_CATEGORY_COLLECTION}`);
    ...
    app.post('/addService', (req, res) => {
        ...
        if (newCategory) {
            categoryCollection.insertOne({ category: newCategory, image })
                .then(result => { res.status(201).send(result.insertedCount > 0) })
                .catch(err => { res.status(500).send(err) });
        }

        serviceCollection.insertOne({ category: category || newCategory, serviceName, description, fee, image })
            .then(result => { res.status(201).send(result.insertedCount > 0) })
            .catch(err => { res.status(500).send(err) });
    });
    ...
    app.get('/serviceItem/:category', (req, res) => {
        ...
        serviceCollection.find({ category })
            .toArray((err, items) => {
                if (err) {
                    res.status(404).send(err);
                } else {
    ...
    app.delete('/deleteCategory', (req, res) => {
        ...
        serviceCollection.deleteMany({ category })
            .then(result => {
                categoryCollection.deleteOne({ category })
